...
def corner_penalty(game, player):
    ...
    corner_weight = 2 
    if is_curr_location_corner(game, game.get_player_location(player)): # giving penalty for moving to corner position
...
def run_away(game, player):
    ...
    distance = get_location_distance(game, player, opp_player) # giving reward for choosing moves far away from opponent's current position
...
class CustomPlayer:
    ...
    def get_move(self, game, legal_moves, time_left):
        ...
        **********************************************************************
        ...
        try:
            ...
            if self.iterative:
                depth = 1 # Assiging depth for iterative deepening
                while True:
            ...
            return move # timeout has occurred, giving back best move 
        return move
    ...
    def minimax(self, game, depth, maximizing_player=True):
        ...
        # when search reaches to max depth, returns heuristic value of game
        if depth == 0:
            # *the max/min players keep track of them
            return (self.score(game, self), None)

         # checking any available legal moves
        legal_moves = game.get_legal_moves()
        if not legal_moves:
            return (game.utility(self), (-1, -1))

        # Maximize/minimize player plays accordingly
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf"), maximizing_player=True):
        ...
        # Maximize/minimize player plays accordingly
        if maximizing_player:
            return self.alphabeta_maximize(game, legal_moves, depth, alpha, beta)
        else:
            return self.alphabeta_minimize(game, legal_moves, depth, alpha, beta)

    def alphabeta_minimize(self, game, legal_moves, depth, alpha, beta):
    ...
    def alphabeta_maximize(self, game, legal_moves, depth, alpha, beta):
